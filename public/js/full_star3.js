        ...
        var CIRCLES = 7;        // the number of circles to draw.  This is also the amount to break the files into, so FFTSIZE/2 needs to divide by this evenly
...
function tick(evt) {
            ...
            for(var i=0; i<CIRCLES; i++) {
                ...
                var g = new createjs.Graphics().beginFill(color).drawPolyStar(centerX, centerY, lastRadius, 8, 0.6, -90);
...
function getWaveImg(thickness) {
			...
			waveShape.graphics.setStrokeStyle(thickness).beginStroke (color).drawPolyStar(0, 0, WAVE_RADIUS, 8, 0.7, -90);
